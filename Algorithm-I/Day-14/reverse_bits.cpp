...
public:
	uint32_t reverseBits(uint32_t n) {
		uint32_t reversed = 0;
		const int n_bits =  sizeof(n) * CHAR_BIT;
		for (int i = 0; i < n_bits; ++i) {
			reversed |= ((n >> (n_bits - i - 1)) & 0x1) << i;
		}
		return reversed;
	}
};
...
{
	Solution solution;
	assert(solution.reverseBits(1 << 31) == 1);
	assert(solution.reverseBits(1 << 30) == 2);
	assert(solution.reverseBits(1 << 30 | 1 << 28) == 10);
